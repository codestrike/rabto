...
app.get('/api/q/:search', function (req, res) {
	...
	query('SELECT i.id, i.title, i.renter, r.name FROM items i LEFT OUTER JOIN renter r on i.renter = r.id WHERE i.title LIKE $1 OR i.description LIKE $1',['%'+search_query+'%'], function (err, result){
